...
var index_esm_1 = index_esm.getShippingEstimateQuantityInSeconds;
...
function isPrototype$1(value) {
  var Ctor = value && value.constructor;
  var proto = typeof Ctor == 'function' && Ctor.prototype || objectProto$12;
...
var isEmpty = function isEmpty(elm) {
  ...
  if ((typeof elm === 'undefined' ? 'undefined' : _typeof(elm)) === 'object' || isPrototype$1(elm)) {
...
function groupPackages(items, criteria, order) {
  return addToPackage(items, criteria, order, function (packages, item) {
    return packages.find(function (pack) {
      return pack.package.index === item.package.index;
    });
  });
}
...
function groupDeliveries(items, criteria, order) {
  return addToPackage(items, criteria, order, function (packages, item) {
    return packages.find(function (pack) {
      ...
      if (criteria.shippingEstimate && criteria.selectedSla && pack.shippingEstimate !== item.shippingEstimate) {
        return false;
      }

      if (criteria.slaOptions) {
        var packSlas = pack.slas.reduce(function (acc, sla) {
          return acc + sla.id;
        }, '');
        var itemSlas = item.slas.reduce(function (acc, sla) {
          return acc + sla.id;
        }, '');
      ...
      var hasDeliveryWindow = pack.deliveryWindow && item.deliveryWindow;

      var areWindowsDifferent = hasDeliveryWindow && pack.deliveryWindow.startDateUtc !== item.deliveryWindow.startDateUtc && pack.deliveryWindow.endDateUtc !== item.deliveryWindow.endDateUtc;

      if (criteria.selectedSla && (hasDeliveryWindow && areWindowsDifferent || !!pack.deliveryWindow !== !!item.deliveryWindow)) {
        return false;
      }

      if (criteria.deliveryChannel && pack.deliveryChannel !== item.deliveryChannel) {
        return false;
      }

      var scheduledSla = getSlaObj(item.slas, item.selectedSla) || getScheduledDeliverySLA(item);

      if (criteria.groupByAvailableDeliveryWindows && hasDeliveryWindows(item.slas) && (!pack.availableDeliveryWindows || !areAvailableDeliveryWindowsEquals(pack.availableDeliveryWindows, scheduledSla.availableDeliveryWindows))) {
        return false;
      }
...
function getItemSelectedSlaPrices(item) {
  var shouldSumDeliveryWindow = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
...
function hasWorstShippingEstimate(pack, item) {
  ...
  return index_esm_1(pack.shippingEstimate) < index_esm_1(item.shippingEstimate);
}

function addToPackage(items, criteria, order, fn) {
  return items.reduce(function (packages, item) {
    var pack = fn(packages, item);
    ...
    if (pack) {
      ...
      var slaPrices = getItemSelectedSlaPrices(item);
    ...
    var selectedSlaObj = getSlaObj(item.slas, item.selectedSla);

    var selectedSlaType = getSlaType(selectedSlaObj, order);

    var scheduledSla = selectedSlaObj || getScheduledDeliverySLA(item);

    var newPackage = _extends({}, getItemSelectedSlaPrices(item, true), {
      items: [item.item],
      package: item.package,
      slas: item.slas,
      pickupFriendlyName: criteria.selectedSla ? item.pickupFriendlyName : undefined,
      seller: criteria.seller ? item.item.seller : undefined,
      address: criteria.selectedSla ? item.address || selectedSlaObj && selectedSlaObj.pickupStoreInfo && selectedSlaObj.pickupStoreInfo.address || undefined : undefined,
      selectedSla: criteria.selectedSla ? item.selectedSla : undefined,
      selectedSlaObj: selectedSlaObj,
      selectedSlaType: selectedSlaType,
      deliveryIds: item.deliveryIds,
      deliveryChannel: criteria.deliveryChannel ? item.deliveryChannel : undefined,
      hasAvailableDeliveryWindows: criteria.groupByAvailableDeliveryWindows ? hasDeliveryWindows(item.slas) : undefined,
      availableDeliveryWindows: criteria.groupByAvailableDeliveryWindows && scheduledSla ? scheduledSla.availableDeliveryWindows : undefined,
      deliveryWindow: criteria.selectedSla ? item.deliveryWindow : undefined,
      shippingEstimate: criteria.selectedSla ? item.shippingEstimate : undefined,
      shippingEstimateDate: criteria.selectedSla ? item.shippingEstimateDate : undefined,
      item: undefined
    });
...
function parcelify(order) {
  var options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
  var _order$items = order.items,
      items = _order$items === undefined ? [] : _order$items,
      _order$marketplaceIte = order.marketplaceItems,
      marketplaceItems = _order$marketplaceIte === undefined ? [] : _order$marketplaceIte,
      _order$packageAttachm = order.packageAttachment,
      packageAttachment = _order$packageAttachm === undefined ? {} : _order$packageAttachm,
      _order$shippingData = order.shippingData,
      shippingData = _order$shippingData === undefined ? {} : _order$shippingData,
      _order$changesAttachm = order.changesAttachment,
      changesAttachment = _order$changesAttachm === undefined ? {} : _order$changesAttachm;


  var criteria = _extends({}, DEFAULT_CRITERIA, options.criteria ? options.criteria : {});

  var packages = packageAttachment && packageAttachment.packages ? packageAttachment.packages : [];
  var logisticsInfo = shippingData && shippingData.logisticsInfo ? shippingData.logisticsInfo : [];
  var selectedAddresses = shippingData && shippingData.selectedAddresses ? shippingData.selectedAddresses : [];
  var changes = changesAttachment && changesAttachment.changesData ? changesAttachment.changesData : [];

  var FIXED_ITEM_INDEX = 0;
  var useItems = criteria.useMarketplaceItems === false || marketplaceItems.length === 0;

  var itemsWithIndex = useItems ? items.map(function (item, index) {
    return _extends({}, item, { index: index });
  }) : marketplaceItems.map(function (item) {
    return _extends({}, item, {
      index: FIXED_ITEM_INDEX
    });
  });

  var packagesWithIndex = packages.map(function (pack, index) {
    return _extends({}, pack, { index: index });
  });

  var itemsCleaned = getNewItems(itemsWithIndex, changes);

  var deliveredItems = getDeliveredItems({
    items: itemsCleaned,
    packages: packagesWithIndex
  });

  var enhancePackage = function enhancePackage(pkg) {
    return hydratePackageWithLogisticsExtraInfo(pkg, logisticsInfo, selectedAddresses);
  };

  deliveredItems.delivered = deliveredItems.delivered.map(enhancePackage);
  deliveredItems.toBeDelivered = deliveredItems.toBeDelivered.map(enhancePackage);

  var deliveredPackages = groupPackages(deliveredItems.delivered, criteria, order);
  var toBeDeliveredPackages = groupDeliveries(deliveredItems.toBeDelivered, criteria, order);

  return deliveredPackages.concat(toBeDeliveredPackages);
}

export { parcelify };
//# sourceMappingURL=parcel.esm.js.map
